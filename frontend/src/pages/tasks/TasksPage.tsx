  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-text-main uppercase">Task Registry</h1>
          <p className="text-sm text-text-muted mt-1 font-medium italic">Manage assignments, deadlines and grading</p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState 
          ...
          icon={<BookOpen />}
          action={isFaculty ? { label: "Create First Task", onClick: handleOpenCreate } : undefined}
        />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredTasks.map((task) => {
            const bcName = typeof task.bootcampId === 'object' ? task.bootcampId.title : 'General Bootcamp';
              ...
              <Card key={task._id} className="group overflow-visible">
                <div className="p-6">
                  <div className="flex justify-between items-start mb-6">
                    <Badge variant={isOverdue ? "destructive" : "secondary"} className="uppercase text-[9px] tracking-widest px-2">
                      {isOverdue ? "Past Due" : "Active"}
                    </Badge>
                    
                    {isFaculty && (
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-text-muted hover:text-brand-accent" onClick={() => handleOpenEdit(task)}>
                          <Edit className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-text-muted hover:text-red-500" onClick={() => handleDelete(task._id)}>
                  ...
                  <h3 className="font-bold text-lg text-text-main leading-tight mb-1 group-hover:text-brand-accent transition-colors">{task.title}</h3>
                  ...
                  <div className="space-y-3 mt-auto">
                    <div className="flex items-center gap-2 text-xs text-text-muted font-medium bg-brand-primary/40 p-2 rounded-lg">
                      ...
                      <span>Due: {task.dueDate ? new Date(task.dueDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : 'No deadline'}</span>
                    </div>
                  ...
                  <div className="mt-6 pt-4 border-t border-brand-border flex items-center justify-between">
                    {!isFaculty ? (
                      <Button className="w-full justify-between group/btn" onClick={() => {
                      ...
                      <Button variant="outline" className="w-full hover:bg-brand-accent hover:text-white transition-colors" onClick={() => navigate(`/dashboard/instructor/submissions?taskId=${task._id}`)}>
                        View Submissions
                      </Button>
                    )}
                  </div>
