...
import { Plus, Activity, BookOpen, AlertCircle, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-text-main uppercase tracking-tight">Division Bootcamps</h1>
          <p className="text-sm text-text-muted mt-1 font-medium italic">Program tracks and curriculum specialized for this unit</p>
        </div>

        {isAdmin && (
          <Button onClick={() => setIsModalOpen(true)} className="shadow-lg shadow-brand-accent/20">
      ...
      {loading ? (
        ...
        <EmptyState 
          ...
          icon={<BookOpen />}
          action={isAdmin ? { label: "Create First Track", onClick: () => setIsModalOpen(true) } : undefined}
        />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBootcamps.map(bootcamp => {
            ...
            return (
              <Card key={bId} className="group h-full overflow-visible border-none bg-white">
                <div className="p-6 h-full flex flex-col">
                  ...
                  <p className="text-sm text-text-muted line-clamp-2 mb-6">
                    {bootcamp.description || 'Comprehensive training track focused on high-level technical mastery and collaboration.'}
                  </p>
