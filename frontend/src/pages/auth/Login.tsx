...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      dispatch(loginSuccess({
        user: { id: user._id || user.id, name: user.name, email: user.email, role: role },
        token: token
      }));
      navigate('/dashboard');
    } catch (err: any) {
      ...
      <div className="max-w-md w-full">
        ...
        <div className="geo-card p-10">
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              <label className="block text-xs font-black text-text-muted mb-2 uppercase tracking-[0.2em]">Educational Email</label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full px-4 py-3 rounded-lg bg-brand-primary/50 border border-brand-border text-text-main focus:border-brand-accent focus:ring-4 focus:ring-brand-accent/5 outline-none transition-all font-medium text-sm" placeholder="student@scholar.astu" />
            </div>
            <div>
              <label className="block text-xs font-black text-text-muted mb-2 uppercase tracking-[0.2em]">Identity Key</label>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full px-4 py-3 rounded-lg bg-brand-primary/50 border border-brand-border text-text-main focus:border-brand-accent focus:ring-4 focus:ring-brand-accent/5 outline-none transition-all font-medium text-sm" placeholder="••••••••" />
            </div>
            
            <div className="text-right">
              <Link to="/forgot-password" className="text-[10px] font-black text-text-muted uppercase tracking-[0.1em] hover:text-brand-accent transition-colors">Forgot Identity Key?</Link>
            </div>

            <Button type="submit" isLoading={loading} className="w-full py-4 text-sm font-black uppercase tracking-widest shadow-lg shadow-brand-accent/20">Sign In to Dashboard</Button>
          </form>

          {error && <p className="text-red-600 text-xs mt-4 text-center font-bold uppercase tracking-wider">{error}</p>}

          <div className="mt-8 pt-6 border-t border-brand-border space-y-4 text-center">
            <p className="text-[11px] text-text-muted font-bold uppercase tracking-widest">New Researcher? <Link to="/register" className="text-brand-accent hover:underline decoration-2 underline-offset-4">Register here</Link></p>
          </div>
